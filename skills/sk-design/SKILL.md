...
- `subagent_type: sk-architectural-advisor` with `topic: <slug>`, `rfc_context: <the scope statement, plus what the dialogue settled — including anything decided about the design's properties>`, `scope_hint: <ui|infra|mixed>`. If the advisor returns the error JSON `{ error: "missing_architecture_context" }`, hard-stop with `error: missing_architecture_context` (see `<hard_stops>`). Reserve `error: subagent_failed` for genuinely malformed advisor output (no JSON fence, a `mode` other than `section_ready`, or a `section` that does not begin with `## Architecture`).
...
**The gate, once.** Run the deterministic gate via Bash — `"${CLAUDE_CONFIG_DIR:-$HOME/.claude}/sidekick/bin/sidekick" check-artifact .sidekick/work/<issue>-<slug>/RFC.md --type rfc` — and parse its JSON. The document is complete by this point (all seven sections present), so this is the only place in Finalisation the gate needs to run — and it runs *before* the second coherence pass, restoring the same economy the first pass's design carried: no model spend on a mechanically broken draft. One call checks frontmatter, required headings, checklist well-formedness, and that every cited `g_n` / `D-NN` resolves. On `verdict: "fail"`, re-dispatch `sk-plan-drafter` with `feedback: <the gate's issues collapsed into one prose summary the drafter can act on>`, append the updated `## Checklist` / `## Tasks` to RFC.md, and re-run the gate — without dispatching the second coherence pass yet. The gate's only error is `missing_artifact`: an orchestration bug, not a draft defect, so halt with `error: artifact_gate_failed` rather than looping.

**Coherence on the complete document.** On gate `verdict: "pass"`, resolve and dispatch the quorum — same `verifiers --surface rfc` resolution, `artifact_path: .sidekick/work/<issue>-<slug>/RFC.md`, `artifact_type: "rfc"` — against the document the gate just cleared. This is `sk-coherence-checker`'s second dispatch in Finalisation: its `rfc` lens checks each task in `## Tasks` against the effective decision set — the check the retired `plan` lens carried before the fold, and the one the deterministic gate cannot do. `check-artifact` only verifies that a task's cited `g_n` / `D-NN` *resolves*; it never asks whether the task *agrees* with what it cites. Skipping this pass would let a task implementing an approach `## Decisions` rejected ship with every reference resolving and every gate green. Running the gate first means this pass always reads the final bytes: whatever the gate forced a redraft to change is what coherence checks.
...
- Any **binding** member `verdict: fail` — re-dispatch `sk-plan-drafter` (not `sk-rfc-drafter` — the design sections already cleared their own coherence pass) with `feedback: <the failing binding members' issues collapsed into one prose summary the drafter can act on>`. Append the updated `## Checklist` / `## Tasks` to RFC.md in place of the prior draft. Re-run the gate, then this quorum — a redraft aimed at a coherence finding can break shape as easily as a redraft aimed at a gate finding can break meaning.
- Cap at 3 `sk-plan-drafter` re-dispatches across the gate above and this quorum, combined. On the third failure, emit `error: rfc_quorum_check_loop_exhausted` and halt. Exhausting the cap on the gate without ever reaching this quorum is a deliberate outcome, not a partial run: a mechanically broken task list halts rather than spending model budget checking its meaning.
...
**Input:** `{ topic: <slug>, rfc_context: <scope_statement + what the dialogue settled, including anything decided about the design's properties>, scope_hint: "ui"|"infra"|"mixed" }`.

**Output:** `{ mode: "section_ready", section, recommendation, confidence, confidence_reason?, off_stack_rejection }` inside a ```json``` fence, or an error JSON of the form `{ error: "missing_architecture_context", reason }`. `section` is the markdown body starting with `## Architecture` — pass it verbatim into `sk-rfc-drafter` as `architecture_section`. `recommendation`, `confidence`, and `off_stack_rejection` are the fields you read for the divergence check and for what you surface; they restate the section's headline, nothing the drafter needs beyond `section`.

**Routing:**

- `{ error: "missing_architecture_context" }` → hard-stop with `error: missing_architecture_context`. Surface the advisor's `reason` (no CLAUDE.md / `.claude/rules/` constraint sources) so the user knows to author a minimal CLAUDE.md before re-running.
- No JSON fence, a `mode` other than `section_ready`, or a `section` that does not begin with `## Architecture` → `error: subagent_failed` (programmer-error path).
- Otherwise — pass `section` through to `sk-rfc-drafter`.

### 4. sk-researcher

**Input — a research brief the orchestrator constructs** (one dispatch per brief):

`{ objective, output_format: "narrative"|"comparison"|"context", background, key_questions, source_policy, scope_boundaries, cap_words: 800 }`

Compose each brief from the settled dialogue state and the survey's `research_hints` — the hint names the angle, the brief makes it explicit: `impl → narrative` (how-to / library findings), `decision → comparison` (a table plus exactly one recommendation; comparison axes go in `key_questions`), `context → context` (citation-anchored prior art). `objective` states what the research must settle; `background` says where the design stands; `scope_boundaries` names what not to research; `source_policy` says what counts as canonical and carries the sources-required discipline.
