...
## Change control
A milestone's scope is what its description names when Opening ends; the seeds and breakdowns Opening files enter it directly. The scope is locked because feature creep has led work into rabbit holes, away from what the milestone set out to do. Work that keeps the repo, the work, or the milestone correct and consistent is not an addition. A doc a change made stale, or a place still stating a premise it retired, is part of that change and is done with it. The test asks what a change left stale or inconsistent. What the outcome still lacks is a discovery instead, and its size decides its route. Filing it as its own issue adds a triage, a plan, and an execution for nothing. Weigh each later addition by its size; the ceremony scales with it.

- A small issue closely tied to work already in the milestone may join on the PM's judgment or the operator's. Small means it fits an existing plan and leaves the outcome as it was. Whoever adds it comments on the issue why, and adds it to the `Plans:` clause when the milestone has one. A `PATCH` replaces the whole description, so that edit carries the rest of it unchanged. The model tells the operator in one line, and the operator can veto.
