...
5. `gh issue create` with the label, the body, and `--milestone` when it has one; then `gh project item-add <board number> --owner @me --url <issue url>`. The board's "Item added to project" workflow sets Status to Backlog. Tell the operator the number in one line.
