...
**The idea:** an AI-support layer that connects a human operator to the meta-process *before* the AI tackles a task — letting them "dial some knobs" up front rather than babysitting during. Candidate knobs: research depth, autonomy level (how hands-off), risk tolerance, budget / effort ceiling, escalation thresholds, **context isolation↔handoff balance** (fidelity-first vs context-budget-first — see `../DESIGN-PRINCIPLES.md` #5).
