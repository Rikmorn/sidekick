...
**Status:** Resolved (2026-08-04, bench-5) — v1 shipped: `sidekick harvest log|list|import` (local inbox at `.sidekick/harvest.jsonl`, gitignored; import produces a `manual: true` case skeleton for adjudication), the ritual referenced from the sk-build/sk-review/sk-design failure paths, and orient surfacing unimported entries. The hosted variant stays parked; if that appetite materialises it is a new note, not this one.
