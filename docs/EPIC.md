...
**Status (2026-06-18):** Architecture settled (ADR-0001/0002/0003; ADR-0004 pending in `1.2`). Toolchain built and working (6 skills + 24 agents + 9 CLI helpers + tier-0 hook). **Phase 0 complete**; **Phase 1 in progress** — `1.1` done 2026-06-18; `1.2`–`1.6` remain. **Re-baselined 2026-06-18** (foundations-then-memory: eval keystone pulled forward into Phase 3; memory consolidated into a dedicated Phase 4; Rules group dropped). Phases 2–5 unstarted. Live state snapshot: [`EPIC-STATE.md`](./EPIC-STATE.md).
...
| **1.2** | **Explorer rethink → ADR-0004 + repair the broken redesign loop** — repurpose `sk-explorer` to a repo-grounding / scope-evidence subagent; move scoping (slug, single-vs-group) into `/sk-design`'s dialogue; **fold F3** (drop the `low\|medium\|high` bucket — it is the "complexity-classification-gate" anti-pattern the discipline doc itself names — for an evidence-grounded signal). **ADR-0004 must also decide how redesign re-enters dialogic `/sk-design`**: today `/sk-design --resume` (sk-goal-verify) and `/sk-design <slug>` (sk-build) are *both* rejected by the explorer's `slug_collision` — the loop is broken end-to-end. | pilot F3; drift sweep; `agentic-loops`; review §loop | 1.1 |
| **1.3** | **Research-quality** — F4 (explicit brief construction, surface briefs, flag thin-repo grounding) + F7 (mark interchangeable specifics substitutable) on `sk-researcher-{context,decision,impl}` + `sk-research-synthesiser`. F4's thin-repo grounding is satisfied by the cached codebase-map (`4.5`) — cross-reference, don't duplicate. → completes the **Researchers** group. | pilot F4/F7 | — |
| **1.4** | **Branch-precheck (F2) + consolidate the agent/CLI split** — orchestrator offers to *create* the branch (precheck stays read-only); and resolve the inconsistency that 4 skills dispatch the `sk-branch-precheck` agent while `sk-decide` calls the CLI directly (recommend: retire the thin agent; all orchestrators call the CLI). → **toolchain audit complete**. | pilot F2; review §inconsistencies | — |
...
| Utility (1) | `sk-branch-precheck` | (light) — pilot F2 | → `1.4` |
