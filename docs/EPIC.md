...
| **3.2** | E7 | **Independent guardrail/output verification** (CoVe-style + seal the gate from the producer + audit constraint-adherence at high stakes) — reuses the existing quorum, consuming the `3.9` registry (same machinery, one implementation). **This is the machinery Phase 4's memory write-gate reuses.** | `verification-autonomy`, `prompting`, `agentic-loops` | ready; relates 3.9 |
...
| **3.9** | — | **Operator-authored verifiers — the `5.1` seam, pulled forward (ADR-0005).** Verifier **contract** (dimensional identity, ONE-JSON deliverable, read-only tooling, `advisory\|binding` tier, model/family field seamed for `3.6`) + **quorum registry** (membership as data; candidate home `.sidekick/config.json` — inside the `0.5` guard surface) + **orchestrator loaders** (sk-review dimension selection; sk-design RFC/PLAN quorums; sk-decide) + a **verifier-authoring skill** embodying the sk-agent-prompts discipline. Judgment verifiers are *advisory* until calibrated through `3.4`; deterministic checks may bind; kernel invariant checkers (structural/crossref) are non-displaceable — operator quorums are strictly additive. **UI dimensions ship as the worked example pack** (closes the ui-audit gap by decision, not by building the 7th command; taste stays advisory per the fuzzy-gate research). Blast radius: every quorum-hardcoding orchestrator + the standing coherence checks reconcile in the same change. **Appended post-baseline — no hard deps for the seam; sequence early (before/with `3.2`), ID-order ≠ execution order here** (same prose-override precedent as Phase 4's header). | ADR-0005, `enforcement-surface` (F7–F9), `verification-autonomy` (rubric refutation; findings 6–7), `prior-art` (gsd eval-planner) | — (seam); binding ← 3.4; relates 3.2/3.6/3.7 |
...
| ID | Was | Item | Sources | Deps |
|---|---|---|---|---|
| **5.1** | E22 | **Generated enforcement — LLM-generated per-project gates** *(narrowed 2026-07-02 by ADR-0005: the contract/registry/authoring seam moved to `3.9`)*. Analyse project/objective → **generate** the quality gates that should hold on the *target* repo, onto the `3.9` surface (the recovered "helpers to create gates on the target project" thread); agents like `sk-maintainability-reviewer` then enforce *those* project gates instead of sidekick's bespoke rules (the retired Rules group was the seed/demonstration of this pattern). Generated gates enter *advisory* and **bind only through independent calibration** (producer≠verifier recurses — F9); intent-anchored regeneration on drift (F7); anti-mock + integration dimensions (F8); tier mobility. | ADR-0002 §5, ADR-0005, `enforcement-surface` (F7–F9), `prior-art` | 3.4, 3.9, 0.5; relates 3.2/3.5 |
