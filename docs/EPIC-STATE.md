...
- **12 CLI helpers** (the "own the loop" kernel, TDD'd): `branch-precheck`, `capabilities`, `check-drift`, `classify-deviation`, `config` (incl. the `gates` resolver), `goal-verdict`, `hash-rfc`, `hooks`, `init`, `reconcile-plan`, `verifiers`, `wave-plan`.

---

## 3. Where we are in the roadmap

**Re-baselined 2026-06-18 → foundations-then-memory; re-baseline 3 (2026-07-03) renumbered Phase 3 to execution order.** Phase shape: **0** foundations/toolchain ✅ · **1** coherence/consistency/loop ✅ (closed 2026-06-18) · **2** codified patterns ✅ (done 2026-07-02 — discipline Rules 9–11) · **3** verification & capability foundations, now in execution order: `3.1` operator-verifier seam ✅ (done 2026-07-03; was `3.9`) → `3.2` guardrail verification → `3.3` eval keystone (was `3.4`) → `3.4` sizing signal (was `3.1`) → `3.5` forcing-function → `3.6` dial → `3.7` cross-family → `3.8` observability → `3.9` zoo reduction · **4** Memory (the dedicated focus) · **5** generative/advanced. Full roadmap + crosswalk: [`EPIC.md`](./EPIC.md). **Next up: `3.2` — independent guardrail/output verification, consuming the `3.1` registry.**
