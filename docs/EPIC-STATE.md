...
**Re-baselined 2026-06-18 → foundations-then-memory.** New phase shape: **0** foundations/toolchain ✅ · **1** coherence/consistency/loop ✅ (closed 2026-06-18) · **2** codified patterns ✅ (done 2026-07-02 — discipline Rules 9–11) · **3** verification & capability foundations (incl. the pulled-forward eval keystone `3.4` + cross-family `3.6` + the appended operator-verifier seam `3.9`, which has no hard deps and is intended early — before/with `3.2`) · **4** Memory (the dedicated focus) · **5** generative/advanced. Full roadmap + crosswalk: [`EPIC.md`](./EPIC.md). **Next up: Phase 3** — the `3.9` seam is flagged early (before/with `3.2`); `3.1`/`3.2` open the phase in ID order.
