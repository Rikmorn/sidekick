...
**Verification (2026-08-04, sealed verifier — separate invocation, spec + artifacts only).** **VERIFIED-WITH-NOTES.** All six gates reproduced independently: 524 tests green; the `3-3-w4` metric frame regenerated with records byte-identical (SHA-256 checked before/after); the lint gate exercised live (broken registry → exit 1 with both error-tier `invalid-metric` findings → restored clean); registry content confirmed against ADR-0008 D1/D2/D5/D6 (18 subjects, 4 metrics, biases match the ledger, D6's named thresholds); metric values recomputed by hand from records.jsonl (sk-coherence-checker 8/8 label-match; sk-rfc-drafter judge n=4 vs adherence n=5 confirmed correct — the date-missing case has no judge assertion). Notes, dispositioned same-day: the docblock's "`--json` contract" phrasing implied a flag that does not exist (reworded — output is unconditionally JSON) and `mechanical-pass-rate` had no direct test (added). Standing notes for successors: `suites`/`totals` rollups coexist beside the metric frame (per-case detail serves findings/manual counts — accepted); adherence is deliverable-shape-only in v1, self-declared in its own bias field, with scope/read-only capture owed to later bench items; grounding's "calibrated" qualifier is bias-text until bench-6 computes certificate checks into the gate.
