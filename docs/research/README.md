...
| **Knowledge layer** | rent vs build vs glue for the repo's own knowledge/visibility layer; substrate for a dual-audience corpus | **glue-with-thin-build** — typed text in git (closed edge vocabulary formalized from the corpus's own) + SQL glue (SQLite/FTS5; validated extractors) + a thin TDD'd compiler (tiers, crosswalk normalization, coverage/gap/diff views) + generated-never-authored rich views (coverage-report pattern; Quarto-class); rent ruled out (stash server-backed+untyped; basic-memory closest — adopt its relation *syntax*, not the tool) | [`knowledge-layer/REPORT.md`](./knowledge-layer/REPORT.md) → ADR-0007 (accepted 2026-07-22) | high (adversarially verified) |

**Execution tracker → [`../EPIC.md`](../EPIC.md)** — the work breakdown, each item a future planning session. The roadmap is organised in **phases** with `{phase}.{item}` IDs (renumbered 2026-06-18 from the old `E#`/tier scheme; legacy `E#` resolve via the crosswalk in EPIC.md). _Reports in this folder reference work by the `E#` IDs current when written — those references are historical and will be dropped as research rotates._ Rationale behind the breakdown → **[`ACTION-PLAN.md`](./ACTION-PLAN.md)** (consolidated across all six research areas; supersedes the A/B/C-only action-map in `SYNTHESIS.md`).
