...
> **Correction note (2026-06-10, E2 session — verified against live docs, several first-hand):** this doc is now partially outdated. (1) **Dynamic Workflows are stable/GA** (May 2026, CC ≥ v2.1.154), *not* research preview — but plan-gated (off-by-default on Pro) and kill-switchable (`disableWorkflows`, no documented fallback). (2) **No mid-run user interaction** in workflows; the docs' own sign-off pattern is "run each stage as its own workflow" — the human stays between stages, in the session. (3) **Workflow resume is same-session only** — cross-session recovery stays hand-rolled. (4) Workflow scripts have **no shell/fs access**; subagents inside workflows always run **`acceptEdits`** (file edits auto-approved) — hooks become the only always-on write gate under that substrate. (5) `/goal`'s evaluator is a **fixed Haiku** judge over conversation surface only (not swappable, can't run commands). Full verified picture + coupling verdict: [`platform-coupling/REPORT.md`](./platform-coupling/REPORT.md); enforcement implications: [`enforcement-surface/REPORT.md`](./enforcement-surface/REPORT.md); cost #1 of the design-emits-workflow candidate (§ below) is hereby **confirmed** (the runtime-model claim flagged for verification holds).
...
- **E12 (cross-family quorum)** — `/goal`'s evaluator is a single, **same-family** (Haiku) judge closing the loop. That's the Track A risk (self-consistent errors ~60% correlated within a family, don't shrink with scale) now shipped into a primitive. `/goal` *does* get **producer ≠ verifier** right (separate invocation). Cross-family quorum reads as the natural upgrade — to weigh when E12 is planned.
- **E13 (eval layer)** — a `/goal` completion-condition (and any loop-exit gate) is essentially an eval rubric. Writing *checkable* conditions is the skill; Code/LLM-judge/Human rubric tiering would harden a fallible judge against gaming / false-satisfaction.
- **E11 (operator-dial)** — the budget/turn ceiling + blast-radius/reversibility governor that `/goal` and `ultracode` leave to manual circuit-breaking (`/goal clear`). **Hooks** look like the deterministic enforcement surface for "irreversible → escalate."
- **E5/E4/E17 (orchestration rules)** — the dynamic-Workflows runtime already encodes several of our findings (plan-in-script-variables ≈ orchestrator-owns-writes; worktree isolation for parallel writes; a hard concurrency cap). The open question shifts toward "what do our rules add *on top of* the runtime," not "build the runtime."
...
2. **Large-task output only** — emitting a workflow for a small task is over-engineering; the dial (E11) chooses (small → prose plan or just-do-it; large/long-horizon → executable workflow).
