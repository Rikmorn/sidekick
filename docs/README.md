...
| [`adr/`](./adr/) | Architecture decisions — **deliberately top-level, never nested in epics**: their scope is the system and they remain binding after the work that spawned them is closed. An ADR's status line names what it supersedes, and the graph carries that as an edge | Record — status transitions (Proposed → Accepted → Superseded) only |
...
**Also records:**
- [`references/`](./references/) — provenance notes for shipped changes from before the work records existed; kept as written.
