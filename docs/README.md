...
| `backlog/` | Designs in progress, one folder per topic holding its `design.md`, written by `sk-design`; a planned design names the issues it became. The flat notes from before the move to GitHub name their issue where live | Ephemeral: a topic folder is deleted when its issues close. The flat notes stay frozen records |
...
**The folder-earning test:** every top-level folder is a distinct retrieval axis. `adr/` is binding, `research/` is topical, `reviews/` and `learnings/` are what was found, `backlog/` is what is being designed, and `work/` is frozen history. Anything retrievable on an existing axis gets no folder of its own.
