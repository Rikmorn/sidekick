...
| [`reviews/`](./reviews/) | **Dated point-in-time assessments of the system** — architecture reviews, regression investigations, calibration audits. Cross-epic by nature (same top-level argument as ADRs); linked to what they examined via `assesses`/`triggered-by` edges | Record — date-prefixed, immutable, superseded by later assessments |

**Dissolving:** [`references/`](./references/) was the pre-item-file workaround for provenance ("promoted out of backlog once executed… so shipped artifacts stay citation-free") — the item completion synthesis subsumes that function entirely. Its occupant is claimed as the E1 item record during platform migration; the folder retires when empty.
