...
**Status:** Accepted as direction (2026-06-07). *Execution is sequenced and gated on the eval layer (EPIC E15) — see Decision.*
...
2. **Make phase internals behavioural** (lean, reasoning-led, not procedural) — this is EPIC E1/E2.
3. **Reduce surface via the E2 audit**, judging each agent by the *context-headroom-vs-fidelity* test (#5), not by raw count. Verifiers and genuinely context-heavy isolations stay; over-decomposed specialists merge.
4. **Make the loop right-sizeable** — full loop for large/risky work, collapse toward behavioural-only for small (the operator-dial, E10).
...
**Gate:** scaffolding may only be shed once the **eval layer (E15)** can verify the leaner harness still delivers. The path to "more behavioural" runs *through* eval — otherwise we trade legible-but-heavy for lean-but-unverified, and "it feels lean enough" is exactly the self-assessment the research says not to trust. **Sequence: E1/E2 (lift behavioural layer) + E15 (eval) → measure → then thin with evidence.**
...
**Negative / costs:** requires the eval layer *first* (can't safely thin before it); the E2 audit is real work; ongoing discipline needed against rigidity-creep in phase internals (the gsd failure mode). Until E15 exists, this ADR changes *direction and sequencing*, not the current shape.
...
- **The behavioural layer's reliability is liftable** by the prompt/tool improvements (E1/E2) — and *measurable* by E15.

## Revisit when
- The eval layer (E15) exists and can measure leaner-harness quality (then execute the thinning).
...
## Links
`../DESIGN-PRINCIPLES.md` (#1–#5, #8) · `../research/prior-art.md` (superpowers vs gsd) · `../research/SYNTHESIS.md` · `../EPIC.md` (E1, E2, E10, E15).
