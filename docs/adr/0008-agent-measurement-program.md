...
> **Amended 2026-09 (CR #1, implemented by #31 in R3):** D6’s gate condition (18 subjects × ≥3 run sets) is superseded by shadow-usage evidence + harvest on home repos, per the R1 restructure. D6 below carries the replacement; Context 1, D1, and D4 are restated to match it. The metric registry, corpora, and calibration protocol stand unchanged — bench is the regression net now, run when prompts change.

## Context

1. **The fleet is unmeasured.** Most of the agent and skill fleet has no eval case at all — `sidekick graph coverage` reports what is measured and `sidekick graph gaps` names what is not, so neither number is restated here. Nothing measures their quality, consistency, adherence, or grounding, and no designed scenario set exists to measure them against. The operator named this — not the visibility layer — as the confidence gate for sk-* self-use: "sk-reassessment comes after we have tools to measure." That framing held until 2026-09; D6 carries the gate as it now stands.
...
**1. Scope is gate-driven, depth-first.** Measure the sk-* execution chain first — the subjects that touch files or gate merges when self-use opens: the executor lane, the review dimensions, the drafters, the quorum checkers, and the orchestrator skills. Which subjects those are is registry data, never prose: `evals/metrics.json` lists them under `subjects`, each tagged with its class, and `sidekick graph coverage` says which of them a corpus feeds. Read the registry for the roster; a name spelled out here would outlive the agent it names. The remaining subjects follow as registry entries + corpora — the machinery is built once.
...
**6. The gate is operator-read, and it informs — it never auto-opens.** The sk-* self-use reassessment opens when the loop has been run for real and the record holds up: three full design → build → review cycles logged on #42, the failures they surfaced worked down through the harvest inbox, and no unexplained regression in the committed run sets. `sidekick` supplies the evidence and not the verdict — `harvest list` for what misfired in use, `graph coverage` for what is measured at all, `eval report` for the per-metric read on the latest run set. The operator reads #42's cycle comments alongside those three and decides. Nothing computes a per-subject green/amber/red, and nothing opens the gate on its own. Thresholds stay in the registry as data, so a regression still has something to be measured against.
...
- The chain subjects are the right regression net for the flows they compose — a real defect in one surfaces in its corpus before it surfaces in use. Confidence for self-use rides on the logged cycles of D6, not on this assumption.
