...
**Status:** Accepted (2026-07-23, operator sign-off — post-implementation: the contracts were built and verified live under `3.3` on 2026-07-07, and acceptance followed a reconciliation pass against the tree. Verified standing at sign-off: case convention live across 7 suites / 43 cases; runner-lane 14/14 on run set `3-3-w4`; `sk-coherence-checker` certificate at `.sidekick/calibrations/` with the decided shape; D7 resolve-time binding degradation in `resolveVerifiers`; the load-bearing headless-fidelity UNCLEAR below was settled by the Wave-1 spike — installed agents DO resolve in `-p` mode, the in-session fallback was never needed). Implements the eval keystone EPIC `3.3` (this body uses post-re-baseline-3 IDs throughout). Consumes ADR-0005's verifier seam (`3.1`); supplies the calibration path ADR-0005 deferred. The contracts here are what `3.4` (sizing signal), `3.7` (cross-family), `3.9` (zoo reduction), Phase 4's write-gate, and `5.1` (generated gates) build on — and the substrate the agent-measurement program ([`../backlog/agent-measurement-program.md`](../backlog/agent-measurement-program.md)) extends.
